...
namespace PeriodicTimetableGeneration
{
    public static class GenerationAlgorithmPESPUtil
    {
        ...
        /// </summary>
        public const int MODULO_DEFAULT = 120;
        ...
        /// <returns>List of constraints.</returns>
        public static List<Constraint> createConstraintSets(List<Constraint> constraints, int size)
        {
            foreach (Constraint constraint in constraints) 
            {
                createConstraintSet(constraint, size);                
            }
            return constraints;
        }
        ...
        /// <param name="size">The size of set.</param>
        public static void createConstraintSet(Constraint constraint, int size)
        {
            // create discrete set according the sequence of number of length "size"
            constraint.DiscreteSet = new Set(createSequenceOfNumber(size), MODULO_DEFAULT);
        ...
        /// <summary>
        /// Creates the Sets for constraints with different period.
        /// </summary>
        /// <param name="constraints">The constraints.</param>
        /// <param name="size">The size of set.</param>
        /// <returns>List of Constraints.</returns>
        public static List<Constraint> createConstraintSetsWithDifferentPeriod(List<Constraint> constraints, int size)
        {
            foreach (Constraint constraint in constraints)
            {
                //createConstraintSet(constraint, size);
                createConstraintSetWithDifferentPeriods(constraint, size);
            }
            return constraints;
        }
        ...
        /// <summary>
        /// Creates the Set for constraint with different periods.
        /// Calculate with frequency of within Modulo Model.
        /// </summary>
        /// <param name="constraint">The constraint.</param>
        /// <param name="size">The size of set.</param>
        public static void createConstraintSetWithDifferentPeriods(Constraint constraint, int size)
        {
            ...
            {
                // new set {60*i .. 60*i + size/modulo_def/period - 1}
                Set newSet = new Set(createSequenceOfNumber(i * period, i * period + size / (MODULO_DEFAULT / period) - 1), MODULO_DEFAULT);
        ...
        private static Constraint mergeEquivalentConstraintGroup(List<Constraint> constraints) 
        {
            ...
            if (constraints.Equals(0)) return null;
